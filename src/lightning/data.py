...
class MultiSceneDataModule(pl.LightningDataModule):
    ...
    def _setup_dataset(self,
        ...
        with open(scene_list_path, 'r') as f:
            ...
            train_list = [name for name in train_list if 'no_img' not in name]  # filter

        if mode == 'train' and self.data_sampler == 'scene_balance':
    ...
    def _build_concat_dataset(
        ...
        print(f"You have in total {len(datasets)} datasets.")
        if mode == 'val':
            if 'all_3testset' in self.val_list_path:
                sub_1 = datasets[0:7]  # T1-T7
                sub_2 = datasets[7:-1]  # T1-T7 hard
                sub_3 = datasets[-1]  # T10
                final = [ConcatDataset(sub_1), ConcatDataset(sub_2), sub_3]
                return final
            else:
                return datasets
        else:
    ...
    def train_dataloader(self):
        ...
        if self.data_sampler == 'scene_balance':
            sampler = RandomConcatSampler(data_source=self.train_dataset,
                                          n_samples_per_subset=self.n_samples_per_subset,
                                          subset_replacement=True,
                                          seed=self.seed)
        else:
            print(self.train_dataset.cummulative_sizes[-1])
