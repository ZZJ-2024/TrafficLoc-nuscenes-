...
class PL_TrafficLoc(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # 诊断用：把各分量 loss 显示到进度条，方便判断是哪一块卡住（短别名）
        if self.trainer.global_rank == 0:
            _alias = {
                'att_loss': 'att', 'coarse_bce_loss': 'cbce',
                'coarse_matching_loss': 'cmat', 'coarse_dense_loss': 'cden',
                'coarse_euc_loss': 'ceuc', 'fine_softmax_loss': 'fsm',
                'fine_matching_loss': 'fmat',
            }
            for k, v in batch['loss_scalars'].items():
                if k in _alias:
                    self.log(_alias[k], v, on_step=True, on_epoch=False,
                             prog_bar=True, logger=False)
        
        # logging (add_scalar 代替 self.log，规避 PL 1.3.5 + accumulate_grad_batches 丢数据的 bug)
