...
class TrafficLocFeatureMatch(nn.Module):
    def __init__(self, config):
        ...
        if self.config['feature_match']['dust_backbone']:
            ...
            coarse_ps = self.config['feature_match']['coarse_patch_size']
            if coarse_ps == 16:
                self.up_conv1=ImageUpSample(256+128,128, align_c=align)
            else:
                # coarse_patch_size=8: coarse 特征来自 stride 8 (enc_dim 维)，与 feat_s8 同分辨率
                # ImageUpSample 会 2x 上采样导致尺寸不匹配，因此使用简单的 channel 融合
                self.up_conv1=nn.Sequential(
                    nn.Conv2d(self.enc_dim+128, 128, 1, bias=False),
                    nn.BatchNorm2d(128),
    ...
    def coarse_matching_dense(self, feat, voxel_feature, data, sel_proj_patch_idx, fps_idx, n_H, n_W):
        ...
        if self.config['feature_match']['coarse_dense_window'] > 0:
            if self.config['feature_match']['coarse_window_gt']:
                ...
                max_index_y = torch.div(max_index_flatten, n_W, rounding_mode='trunc')
                sim_matrix = sim_matrix.reshape(-1, n_H, n_W) # [L, nH, nW]
    ...
    def train_forward(self, data):
        ...
        if 'feat' not in data:
            if self.config['feature_match']['dust_backbone']:
                ...
                feat=self.encoder_embed_img(feat) # BC(HW), DUSt3R stride=16, 160x320 → 10x20 = 200 token
        ...
        if self.config['feature_match']['point_backbone']=='pimae':
            ...
            # KPConv 的邻居索引绑定了单点云结构，batch_size>1 时拆开逐样本处理
            kpconv_dict = data['kpconv_dict']
            batch_size_kp = data['voxel_points'].shape[0]

            pc_decode_3_list, pc_feature_middle_list = [], []
            center_points_list = []

            for b in range(batch_size_kp):
                single_dict = {}
                for key, val in kpconv_dict.items():
                    if isinstance(val, list):
                        single_dict[key] = []
                        for v in val:
                            single_dict[key].append(v[b])
                    elif isinstance(val, torch.Tensor):
                        single_dict[key] = val[b]
                    else:
                        single_dict[key] = val

                pc_feature_set = self.point_backbone(single_dict)

                pc_decode_3_list.append(F.normalize(pc_feature_set[-4], dim=1, p=2))
                pc_feature_middle_list.append(F.normalize(self.pc_feature_layer(pc_feature_set[-1]), dim=1, p=2))
                center_points_list.append(single_dict['points'][-1])  # [1280, 3]

            pc_decode_3 = torch.stack(pc_decode_3_list, dim=0)          # B x 10240 x 64
            voxel_feature = torch.stack(pc_feature_middle_list, dim=0)  # B x 1280 x 128
            center_points = torch.stack(center_points_list, dim=0)      # B x 1280 x 3
            fine_pt_feat = pc_decode_3                                   # B x 10240 x 64

            fps_idx = data['fps_idx']
        else:
            raise ValueError
        ...
        if self.config['feature_match']['att_loss'] and self.training:
            ...
            # 加 epsilon 防止某些 batch 内 GAL mask 全为空导致 0/0 = NaN，进而污染整条 loss
            att_loss = total_loss / (total_sample + 1e-9)
        ...
        if self.config['feature_match']['fine_match'] and self.config['feature_match']['point_backbone'] == 'kpconv':
            ...
            if coarse_patch_size == 16:
                ...
                # coarse_patch_size=8: ori_feat 与 feat_s8 同分辨率，直接 channel concat 融合
                ori_feat_s8 = rearrange(ori_feat, 'b (h w) c -> b c h w', h=n_H)
                up_feat_s8 = self.up_conv1(torch.cat([ori_feat_s8, feat_s8], dim=1))
        ...
        if self.training and self.config['feature_match']['coarse_match']:
            if self.config['feature_match']['coarse_match_type'] == 'bce_dense':
                ...
                coarse_pc_score = torch.sigmoid(coarse_pc_score).squeeze(1)
            ...
            if self.config['feature_match']['fine_match']:
                if self.config['feature_match']['coarse_match_type'] in ['dense', 'bce_dense']:
                    ...
                    proj_xy_int_s2 = torch.div(proj_xy_int, 2, rounding_mode='trunc')
                ...
                if self.config['feature_match']['fine_match'] and matched_patch.shape[0] > 0:
                    ...
                    if self.config['feature_match']['fine_loss_type'] in ['log_contrastive', 'fine_softmax', 'circle']:
                        ...
                        delta_xy = torch.stack(
                                [delta_xy % patch_size, torch.div(delta_xy, patch_size, rounding_mode='trunc')],
                                dim=1)
                        ...
                        max_index_y = torch.div(max_index_flatten, patch_size, rounding_mode='trunc')
