...
class ImageEncoder(nn.Module):
    def __init__(self, load_pretrain=False):
        ...
        if not load_pretrain:
            ...
            self.backbone = resnet34(in_channels=3, pretrained=True, progress=True)
...
class DepthEncoder(nn.Module):
    def __init__(self):
        ...
        self.backbone = resnet18(in_channels=3, pretrained=False, progress=True)
...
class ImageUpSample(nn.Module):
    def __init__(self,in_channel,output_channel,align_c=False):
        super(ImageUpSample,self).__init__()
        self.up=nn.Upsample(scale_factor=2,mode='bilinear',align_corners=align_c)
...
class ImageUpSampleNearest(nn.Module):
    def __init__(self,in_channel,output_channel,align_c=False):
        super(ImageUpSampleNearest,self).__init__()
...
class OneImageUpSample(nn.Module):
    def __init__(self,in_channel,output_channel):
        super(OneImageUpSample,self).__init__()
...
class OneImageUpSampleDummy(nn.Module):
    def __init__(self,in_channel,output_channel,align_c=False):
        super(OneImageUpSampleDummy,self).__init__()
        self.up=nn.Upsample(scale_factor=2,mode='bilinear',align_corners=align_c)
        #self.up=nn.ConvTranspose2d(in_channel,in_channel,kernel_size=3,stride=2)
        self.conv=nn.Sequential(ResidualConv(in_channel,output_channel),ResidualConv(output_channel,output_channel))
        '''self.conv=nn.Sequential(nn.Conv2d(in_channel,output_channel,1,bias=False),nn.BatchNorm2d(output_channel),nn.ReLU(),
