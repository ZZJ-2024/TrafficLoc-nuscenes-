...
class Embeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.embed_dim = self.config['feature_match']['pt_embed_dim']
        self.point_feat_dim = self.config['feature_match']['pt_point_feat_dim']
        self.num_proxy = self.config['feature_match']['pt_num_proxy']
        
        self.raw_point_mlp = MiniPointNet(self.point_feat_dim, self.embed_dim)

        # GroupPointTransformer is Attention Aggregation
        # we perform feature extraction similar to SO-Net
        self.group_transformer_0 = GroupPointTransformer(self.embed_dim, self.embed_dim)
        self.point_mlp_0 = MiniPointNet(2 * self.embed_dim, self.embed_dim)

        self.group_transformer_1 = GroupPointTransformer(self.embed_dim, self.embed_dim)
        self.point_mlp_1 = MiniPointNet(2 * self.embed_dim, self.embed_dim)

        self.group_transformer_node = GroupPointTransformer(self.embed_dim, self.embed_dim)

        # KnnPointTransformer is standard Point Transformer
        self.knn_transformers = nn.ModuleList()
        self.knn_transformers.append(KnnPointTransformer(self.embed_dim, self.embed_dim, k=16))
        self.knn_transformers.append(KnnPointTransformer(self.embed_dim, self.embed_dim, k=16))
        self.knn_transformers.append(KnnPointTransformer(self.embed_dim, self.embed_dim, k=16))

        self.group_transformer_proxy = GroupPointTransformer(self.embed_dim, self.embed_dim)
    ...
    def forward(self, x, node, idx):
        ...
        proxy = node[:, :, :self.num_proxy]
        proxy_feat = node_feat[:, :, :self.num_proxy]
        with torch.no_grad():
...
class Attention(nn.Module):
    def __init__(self, config):
        ...
        self.num_head = config['feature_match']['pt_num_head']
        self.embed_dim = config['feature_match']['pt_embed_dim']
        self.attention_dropout = config['feature_match']['pt_attention_dropout']
        
        self.num_attention_heads = self.num_head
        ...
        self.proj_dropout = nn.Dropout(self.attention_dropout)
...
class Mlp(nn.Module):
    def __init__(self, config):
        ...
        self.mlp_dim = config['feature_match']['pt_mlp_dim']
        self.embed_dim = config['feature_match']['pt_embed_dim']
        self.mlp_dropout = config['feature_match']['pt_mlp_dropout']
        
        self.fc1 = nn.Linear(self.embed_dim, self.mlp_dim)
        self.fc2 = nn.Linear(self.mlp_dim, self.embed_dim)
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.attention_norm = nn.LayerNorm(self.embed_dim, eps=1e-6)
...
class PointTransformer(nn.Module):
    def __init__(self, config):
        ...
        for _ in range(config['feature_match']['pt_num_sa_layer']):
